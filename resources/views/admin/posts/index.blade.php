...
<section id="posts">
  ...
  @if (session('success'))
  <p id="success-message" class="text-white px-4 py-2 rounded bg-green-500 transition-opacity duration-500">
      {{ session('success') }}
  </p>
  <script>// message fade out
      const el = document.getElementById('success-message');
      if (el) {
          setTimeout(() => {
              el.classList.add('opacity-0');
              setTimeout(() => el.remove(), 500); // fade শেষে remove
          }, 2000);
      }
  </script>
  @endif
